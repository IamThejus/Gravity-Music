...
- **`name` is required by the form** (it was optional in the first version). The column stays **nullable** so rows written before that still read back — query with `coalesce(name, '—')` rather than assuming non-null. It's a display name the user types, not an account identifier; the install itself stays anonymous.
- **`category`** is `idea` / `bug` / `other`, enforced by the `feedback_category_check` constraint and mirrored by the `FeedbackCategory` enum's `wire` values. Change one and you must change the other — a drift makes every insert fail. It defaults to `'other'` at both layers so the form is never blocked on a choice, and so pre-existing rows backfill cleanly.
